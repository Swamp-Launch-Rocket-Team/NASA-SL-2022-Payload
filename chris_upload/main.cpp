...
int main( int argc, char* argv[] )
{
    ...
    cv::Mat img_object = cv::imread( "../images/477.jpg" );
    std::cout << "Done!" << std::endl;

    std::cout <<"Reading reference image..." << std::endl;
    cv::Mat img_scene = cv::imread( "../images/4000Map.png" );
    std::cout << "Done!" << std::endl;
    ...
    cv::Point2f temp = cv::Point2f(200, 120);
    for (int i = 0; i <= 20; i++)
    {
        for (int j = 0; j <= 20; j++)
        {
            circle(gridMat, temp, 20, (0, 0, 255), cv::LineTypes::FILLED);
